...
def pingController() 
{
	...
	// First update the current status if the mutliplexer did not replied since more than 5 min
	def currentState = device.currentState("multiplexerStatus");
	if (currentState && state.lastReportedStatus)
	{
		log.debug "Ping mutliplexer (last status reported: ${currentState.value} @ ${state.lastReportedStatus}, initially reported on ${currentState.date})"
		if (currentState.value == "online")
		use( TimeCategory ) {
			if(Date.parseToStringDate(state.lastReportedStatus) + 5.minutes < new Date())
			{
				log.debug "Mutliplexer didn't replied to ping since more than 5 minutes, report it as offline."
	...
	{
		log.debug "Ping mutliplexer (Status was not reported yet)"
	}
